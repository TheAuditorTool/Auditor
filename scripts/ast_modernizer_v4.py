...
class ASTModernizerTransformer(m.MatcherDecoratableTransformer):
    ...
    def _create_safe_check(
        self, node_arg: cst.BaseExpression, value_type: cst.Name | cst.Tuple
        ...
        const_check = cst.Call(
            ...
            args=[
                cst.Arg(node_arg),
                cst.Arg(cst.Attribute(value=cst.Name("ast"), attr=cst.Name("Constant"))),
            ],
        )
        ...
        value_check = cst.Call(
            func=cst.Name("isinstance"),
            args=[
                cst.Arg(cst.Attribute(value=node_arg, attr=cst.Name("value"))),
                cst.Arg(value_type),
            ],
        )
    ...
    @m.leave(
        m.Call(
            ...
            args=[m.Arg(), m.Arg(m.Attribute(value=m.Name("ast"), attr=m.Name("Str")))],
        )
    )
    ...
    @m.leave(
        m.Call(
            func=m.Name("isinstance"),
            args=[m.Arg(), m.Arg(m.Attribute(value=m.Name("ast"), attr=m.Name("Num")))],
        )
    )
    def replace_ast_num_isinstance(
        ...
        types_tuple = cst.Tuple(
            elements=[cst.Element(cst.Name("int")), cst.Element(cst.Name("float"))]
        )
    ...
    @m.leave(
        m.Call(
            func=m.Name("isinstance"),
            args=[m.Arg(), m.Arg(m.Attribute(value=m.Name("ast"), attr=m.Name("Bytes")))],
        )
    )
    ...
    @m.leave(
        m.Call(
            func=m.Name("isinstance"),
            args=[m.Arg(), m.Arg(m.Attribute(value=m.Name("ast"), attr=m.Name("NameConstant")))],
        )
    )
    ...
    @m.leave(m.Attribute(attr=m.Name("s")))
    def replace_dot_s_with_dot_value(
        ...
        if isinstance(updated_node.value, cst.Name):
            ...
            if (
                var_name in self.definite_ast_vars
                or self.inside_ast_context
                or (self.current_for_target and var_name == self.current_for_target)
            ):
            ...
            if (
                base in self.definite_ast_vars
                or self.inside_ast_context
                or (self.current_for_target and base == self.current_for_target)
            ):
                self.stats.attr_s_fixed += 1
                return updated_node.with_changes(attr=cst.Name("value"))
    ...
    @m.leave(m.Attribute(attr=m.Name("n")))
    ...
    @m.leave(
        m.Call(
            func=m.Name("hasattr"),
            args=[m.Arg(), m.Arg(m.SimpleString('"s"') | m.SimpleString("'s'"))],
        )
    )
    ...
    @m.leave(
        m.Call(
            func=m.Name("hasattr"),
            args=[m.Arg(), m.Arg(m.SimpleString('"n"') | m.SimpleString("'n'"))],
        )
    )
    ...
    @m.leave(m.Call(func=m.Name("isinstance"), args=[m.Arg(), m.Arg(m.Tuple())]))
    def simplify_tuple_isinstance(
        ...
        for element in tuple_arg.elements:
            if isinstance(element.value, cst.Attribute):
                if m.matches(element.value, m.Attribute(value=m.Name("ast"))):
                    attr_name = (
                        element.value.attr.value if hasattr(element.value.attr, "value") else None
                    )
        ...
        if has_constant and has_deprecated:
                ...
                if isinstance(element.value, cst.Attribute):
                    if m.matches(element.value, m.Attribute(value=m.Name("ast"))):
                        attr_name = (
                            ...
                            if hasattr(element.value.attr, "value")
                            else None
                        )
            ...
            if len(new_elements) == 1:
                return cst.Call(
                    func=cst.Name("isinstance"),
                    args=[cst.Arg(node_arg), cst.Arg(new_elements[0].value)],
                )

            elif new_elements:
                return updated_node.with_changes(
                    args=[updated_node.args[0], cst.Arg(cst.Tuple(elements=new_elements))]
                )
    ...
    def visit_Assign(self, node: cst.Assign) -> None:
        ...
        if (
            m.matches(
                node.value, m.Call(func=m.Attribute(value=m.Name("ast"), attr=m.Name("walk")))
            )
            or m.matches(
                node.value,
                m.Call(
                    func=m.Name("list"),
                    args=[
                        m.Arg(m.Call(func=m.Attribute(value=m.Name("ast"), attr=m.Name("walk"))))
                    ],
                ),
            )
            or m.matches(
            ...
            while current_comp is not None:
                if m.matches(
                    current_comp.iter,
                    m.Call(func=m.Attribute(value=m.Name("ast"), attr=m.Name("walk"))),
                ):
                    contains_ast_walk = True
                    break
    ...
    def visit_For(self, node: cst.For) -> None:
        ...
        if m.matches(node.iter, m.Call(func=m.Attribute(value=m.Name("ast"), attr=m.Name("walk")))):
            self.inside_ast_context = True
    ...
    def leave_For(self, original_node: cst.For, updated_node: cst.For) -> cst.For:
        ...
        is_ast_walk_loop = m.matches(
            original_node.iter, m.Call(func=m.Attribute(value=m.Name("ast"), attr=m.Name("walk")))
        ) or (
            isinstance(original_node.iter, cst.Name)
            and original_node.iter.value in self.ast_walk_result_vars
        )
...
def process_file(
    filepath: Path, stats: ModernizationStats, dry_run: bool = False, verbose: bool = False
) -> bool:
    """
...
Examples:
  ...
  python ast_modernizer_v2.py --verbose --dry-run --target-dir ./theauditor/
    ...
    parser.add_argument(
        "--include-tests", action="store_true", help="Also process test files (normally skipped)"
    )
