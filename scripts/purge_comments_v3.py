...
class DocstringTruncateTransformer(cst.CSTTransformer):
    ...
    ) -> tuple[bool, cst.BaseExpression | None]:
    ...
    def _process_body(
        self, body: cst.IndentedBlock | cst.SimpleStatementSuite
...
class CommentPurgeTransformer(cst.CSTTransformer):
    ...
    def __init__(
        self, filename: str, preserve_semantic: bool = False, preserve_copyright: bool = False
    ) -> None:
        super().__init__()
        self.filename = filename
    ...
    def _log_comment(self, node: cst.CSTNode, comment_content: str, context_type: str) -> bool:
        """
    ...
    def leave_EmptyLine(
        ...
        if updated_node.comment is not None:
            should_keep = self._log_comment(original_node, updated_node.comment.value, "block")
            if not should_keep:
    ...
    def leave_TrailingWhitespace(
        ...
        if updated_node.comment is not None:
            should_keep = self._log_comment(original_node, updated_node.comment.value, "inline")
            if not should_keep:
                return updated_node.with_changes(comment=None)
...
def process_file(
    ...
    try:
        ...
        transformer = CommentPurgeTransformer(
            filepath, preserve_semantic=preserve_semantic, preserve_copyright=preserve_copyright
        )
...
def purge_directory(
        ...
        for file in files:
            if file.endswith(".py"):
                ...
                removed, debt, preserved, docstrings = process_file(
                    ...
                    dry_run=dry_run or extract_only,
                )
    ...
    if debt_log:
        ...
        for marker, count in sorted(
            debt_by_marker.items(), key=lambda x: (MARKER_PRIORITY.get(x[0], 99), -x[1])
        ):
...
""",
    ...
    parser.add_argument(
        "--preserve-copyright", action="store_true", help="Keep copyright/license headers"
    )
    ...
    parser.add_argument(
        "--skip", type=str, default="", help="Additional directories to skip (comma-separated)"
    )
