...
class FileContextTransformer(m.MatcherDecoratableTransformer):
    ...
    @m.leave(
        m.FunctionDef(
            params=m.Parameters(
                params=[m.Param(name=m.Name("tree")), m.Param(name=m.Name("parser_self"))]
            )
        )
    )
    def update_extractor_signature(
        ...
        new_param = cst.Param(
            name=cst.Name("context"), annotation=cst.Annotation(annotation=cst.Name("FileContext"))
        )

        new_params = updated_node.params.with_changes(params=[new_param])
                ...
                m.SimpleStatementLine(
                    body=[
                        m.Assign(
                            ...
                            value=m.Call(
                                func=m.Attribute(value=m.Name("tree"), attr=m.Name("get")),
    ...
    def replace_ast_walk_loop(self, original_node: cst.For, updated_node: cst.For) -> cst.For:
        ...
        if node_type:
            ...
            new_iter = cst.Call(
                func=cst.Attribute(value=cst.Name("context"), attr=cst.Name("walk_tree")), args=[]
            )
    ...
    def _extract_isinstance_node_type(self, body: cst.IndentedBlock) -> cst.BaseExpression | None:
        ...
        if m.matches(
            first_stmt,
            m.If(test=m.Call(func=m.Name("isinstance"), args=[m.Arg(m.Name()), m.Arg()])),
        ):
    ...
    def replace_tree_references(
        ...
        if updated_node.value in ["actual_tree", "ast_tree"]:
            return cst.Attribute(value=cst.Name("context"), attr=cst.Name("tree"))

        return updated_node

    @m.leave(m.Call(func=m.Name("_build_function_ranges"), args=[m.Arg()]))
    def replace_function_ranges(
        ...
        return cst.Attribute(value=cst.Name("context"), attr=cst.Name("function_ranges"))
                    ...
                    import_stmt = cst.SimpleStatementLine(
                        body=[
                            cst.ImportFrom(
                                ...
                                names=[cst.ImportAlias(name=cst.Name("FileContext"))],
                            )
                        ]
                    )
...
def process_file(
    filepath: Path, stats: TransformationStats, dry_run: bool = False, verbose: bool = False
) -> bool:
    """
