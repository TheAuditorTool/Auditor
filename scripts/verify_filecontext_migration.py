            ...
            if self.parser_self_files:
                ...
                for ext in self.extractors:
                    if ext.uses_parser_self:
                        ...
                        print("  Usages:")
                        for usage in ext.parser_self_usages[:5]:
                            print(f"    - {usage}")
...
class ExtractorAnalyzer(ast.NodeVisitor):
    ...
    def visit_For(self, node: ast.For):
        ...
        if (
            ...
            and node.iter.func.value.id == "ast"
        ):
            body = node.body
            if body:
                first_stmt = body[0]
                ...
                if not isinstance(first_stmt, ast.If):
                    pattern = (
                        f"Line {self.current_line}: ast.walk with logic before isinstance check"
                    )
                    ...
                    if not (
                        ...
                        pattern = (
                            f"Line {self.current_line}: ast.walk with non-isinstance condition"
                        )
                        self.complex_walks.append(pattern)
    ...
    def visit_Call(self, node: ast.Call):
        ...
        if call_name and call_name in self.extractor_names:
            ...
            if len(node.args) >= 2:
                ...
                self.calls.append({"line": line, "name": call_name, "pattern": arg_pattern})
...
def find_extractor_callers(
    root_dir: Path, extractor_names: set[str], report: VerificationReport
) -> None:
    ...
    for search_path in search_paths:
        ...
        for py_file in search_path.rglob("*.py"):
            if (
                ...
                or py_file.name.endswith(".bak")
                or str(py_file) in processed_files
            ):
                continue
...
def verify_migration_readiness(target_dir: Path) -> VerificationReport:
    ...
    if target_dir.is_file():
        ...
        python_files = [
            f for f in python_files if not f.name.startswith("__") and not f.name.endswith(".bak")
        ]
