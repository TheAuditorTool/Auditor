...
def find_logic_issues(context: StandardRuleContext) -> List[StandardFinding]:
    ...
    try:
        ...
        # ========================================================
        # Look for arithmetic operations on money-related variables
        cursor.execute("""
            SELECT DISTINCT a.file, a.line, a.target_var, a.source_expr
            FROM assignments a
            WHERE (a.target_var LIKE '%price%'
                   OR a.target_var LIKE '%cost%'
                   OR a.target_var LIKE '%amount%'
                   OR a.target_var LIKE '%total%'
                   OR a.target_var LIKE '%balance%'
                   OR a.target_var LIKE '%payment%'
                   OR a.target_var LIKE '%fee%'
                   OR a.target_var LIKE '%money%'
                   OR a.target_var LIKE '%charge%'
                   OR a.target_var LIKE '%refund%')
        ...
        cursor.execute("""
            ...
            WHERE f.callee_function IN ('parseFloat', 'float', 'Number.parseFloat')
              AND (f.argument_expr LIKE '%price%'
                   OR f.argument_expr LIKE '%cost%'
                   OR f.argument_expr LIKE '%amount%'
                   OR f.argument_expr LIKE '%balance%'
                   OR f.argument_expr LIKE '%payment%')
        ...
        cursor.execute("""
            ...
            FROM function_call_args f
            WHERE f.callee_function IN ('open', 'fopen', 'fs.open', 'fs.createReadStream', 'fs.createWriteStream')
              AND NOT EXISTS (
                  SELECT 1 FROM function_call_args f2
                  WHERE f2.file = f.file
                    AND f2.caller_function = f.caller_function
                    AND f2.callee_function IN ('close', 'fclose', 'end', 'destroy')
                    AND f2.line > f.line
              )
            ORDER BY f.file, f.line
        """)
        ...
        for file, line, open_func, in_function in cursor.fetchall():
            ...
            cursor.execute("""
                ...
                WHERE path = ?
                  AND line BETWEEN ? AND ?
                  AND (name = 'with' OR name = '__enter__' OR name = 'finally')
            """, (file, line - 2, line + 2))
        ...
        cursor.execute("""
            ...
            WHERE f.callee_function IN (
              ...
              AND NOT EXISTS (
                  SELECT 1 FROM function_call_args f2
                  WHERE f2.file = f.file
                    AND f2.callee_function IN ('commit', 'rollback', 'end')
                    AND f2.line > f.line
