...
METADATA = RuleMetadata(
    name="graphql_sensitive_fields", category="security", execution_scope="database"
)
...
def check_sensitive_fields(context: StandardRuleContext) -> list[StandardFinding]:
    ...
    for row in cursor.fetchall():
        ...
        is_sensitive = any(
            sensitive in field_name_lower for sensitive in SensitiveFieldPatterns.SENSITIVE_NAMES
        )
            ...
            if directives_json:
                ...
                try:
                    ...
                    for directive in directives:
                        if any(
                            p in directive.get("name", "")
                            for p in ["@private", "@internal", "@deprecated"]
                        ):
                            has_protection = True
                            break
