...
from .python_memory_cache import PythonMemoryCacheLoader
...
class MemoryCache:
    ...
    def preload(
        ...
        try:
            ...
            # Python-specific ORM metadata (Phase 3 parity) - delegated to python_loader
            self.python_loader.load_python_data(cursor)
    ...
    # Python-specific proxy methods (delegate to python_loader)
    # These maintain backward compatibility while keeping implementation in separate module
    ...
    def get_python_model_for_var(
        ...
        return self.python_loader.get_python_model_for_var(
            file_path, function_names, var_name, existing_bindings
        )

    def get_python_relationships(self, model_name: str) -> List[Dict[str, Any]]:
        """Return relationship definitions for a Python ORM model."""
        return self.python_loader.get_python_relationships(model_name)

    def get_python_fk_fields(self, model_name: str) -> List[Dict[str, Any]]:
        """Return foreign key field definitions for a model."""
        return self.python_loader.get_python_fk_fields(model_name)

    # Properties to access Python data structures (for backward compatibility)
    @property
    def python_model_names(self):
        return self.python_loader.python_model_names

    @property
    def python_table_to_model(self):
        return self.python_loader.python_table_to_model

    @property
    def python_param_types(self):
        return self.python_loader.python_param_types

    @property
    def python_relationship_aliases(self):
        return self.python_loader.python_relationship_aliases

    @property
    def python_fk_fields(self):
        return self.python_loader.python_fk_fields

    @property
    def type_annotations(self):
        return self.python_loader.type_annotations

    @property
    def type_annotations_by_file(self):
        return self.python_loader.type_annotations_by_file

    @property
    def python_orm_models(self):
        return self.python_loader.python_orm_models

    @property
    def python_orm_fields(self):
        return self.python_loader.python_orm_fields

    @property
    def orm_relationships(self):
        return self.python_loader.orm_relationships

    @property
    def python_routes(self):
        return self.python_loader.python_routes

    @property
    def python_blueprints(self):
        return self.python_loader.python_blueprints

    @property
    def python_validators(self):
        return self.python_loader.python_validators

    @property
    def python_orm_models_by_file(self):
        return self.python_loader.python_orm_models_by_file

    @property
    def python_orm_models_by_name(self):
        return self.python_loader.python_orm_models_by_name

    @property
    def python_orm_fields_by_model(self):
        return self.python_loader.python_orm_fields_by_model

    @property
    def orm_relationships_by_source(self):
        return self.python_loader.orm_relationships_by_source

    @property
    def orm_relationships_by_target(self):
        return self.python_loader.orm_relationships_by_target

    @property
    def python_routes_by_file(self):
        return self.python_loader.python_routes_by_file

    @property
    def python_routes_by_framework(self):
        return self.python_loader.python_routes_by_framework

    @property
    def python_validators_by_model(self):
        return self.python_loader.python_validators_by_model

    @property
    def python_validators_by_file(self):
        return self.python_loader.python_validators_by_file

    @property
    def python_blueprints_by_name(self):
        return self.python_loader.python_blueprints_by_name
    ...
    def get_assignments_for_function(self, file_path: str, function_name: str) -> List[Dict[str, Any]]:
        ...
        if assignments:
            return assignments
        for candidate in self.python_loader._generate_function_name_candidates(function_name):
