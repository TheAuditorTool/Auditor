...
class BashCommandsTable:
    ...
    @staticmethod
    def get_by_wrapped_command(
        cursor: sqlite3.Cursor, wrapped_command: str
    ) -> list[dict[str, Any]]:
...
class BashFunctionsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "bash_functions",
            ["file", "line", "end_line", "name", "style", "body_start_line", "body_end_line"],
        )
        cursor.execute(query)
        return [
            dict(
...
class BashPipesTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "bash_pipes",
            ["file", "line", "pipeline_id", "position", "command_text", "containing_function"],
        )
        cursor.execute(query)
        return [
            dict(
...
class BashRedirectionsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "bash_redirections",
            ["file", "line", "direction", "target", "fd_number", "containing_function"],
        )
        cursor.execute(query)
        return [
            dict(
...
class BashVariablesTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "bash_variables",
            ["file", "line", "name", "scope", "readonly", "value_expr", "containing_function"],
        )
        cursor.execute(query)
        return [
            dict(
...
class GoCapturedVarsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "go_captured_vars",
            ["file", "line", "goroutine_id", "var_name", "var_type", "is_loop_var"],
        )
        cursor.execute(query)
        return [
            dict(
...
class GoChannelOpsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "go_channel_ops", ["file", "line", "channel_name", "operation", "containing_func"]
        )
        cursor.execute(query)
        return [
            dict(
                zip(
                    ["file", "line", "channel_name", "operation", "containing_func"],
                    row,
                    strict=True,
                )
            )
...
class GoChannelsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "go_channels", ["file", "line", "name", "element_type", "direction", "buffer_size"]
        )
        cursor.execute(query)
        return [
            dict(
                zip(
                    ["file", "line", "name", "element_type", "direction", "buffer_size"],
                    row,
                    strict=True,
                )
            )
...
class GoConstantsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "go_constants", ["file", "line", "name", "value", "type", "is_exported"]
        )
        cursor.execute(query)
        return [
            dict(zip(["file", "line", "name", "value", "type", "is_exported"], row, strict=True))
            for row in cursor.fetchall()
        ]
    ...
    def get_by_file(cursor: sqlite3.Cursor, file: str) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "go_constants",
            ["file", "line", "name", "value", "type", "is_exported"],
            where="file = ?",
        )
        cursor.execute(query, (file,))
        return [
...
class GoDeferStatementsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "go_defer_statements", ["file", "line", "containing_func", "deferred_expr"]
        )
        cursor.execute(query)
        return [
            dict(zip(["file", "line", "containing_func", "deferred_expr"], row, strict=True))
            for row in cursor.fetchall()
        ]
    ...
    def get_by_file(cursor: sqlite3.Cursor, file: str) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "go_defer_statements",
            ["file", "line", "containing_func", "deferred_expr"],
            where="file = ?",
        )
        cursor.execute(query, (file,))
        return [
    ...
    @staticmethod
    def get_by_containing_func(
        cursor: sqlite3.Cursor, containing_func: str
    ) -> list[dict[str, Any]]:
        """Get rows by containing_func."""
        query = build_query(
            "go_defer_statements",
            ["file", "line", "containing_func", "deferred_expr"],
            where="containing_func = ?",
        )
...
class GoErrorReturnsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        cursor.execute(query)
        return [
            dict(zip(["file", "line", "func_name", "returns_error"], row, strict=True))
            for row in cursor.fetchall()
        ]
    ...
    def get_by_file(cursor: sqlite3.Cursor, file: str) -> list[dict[str, Any]]:
        """Get rows by file."""
        query = build_query(
            "go_error_returns", ["file", "line", "func_name", "returns_error"], where="file = ?"
        )
        cursor.execute(query, (file,))
        return [
            dict(zip(["file", "line", "func_name", "returns_error"], row, strict=True))
    ...
    def get_by_func_name(cursor: sqlite3.Cursor, func_name: str) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "go_error_returns",
            ["file", "line", "func_name", "returns_error"],
            where="func_name = ?",
        )
...
class GoFuncReturnsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "go_func_returns",
            ["file", "func_name", "func_line", "return_index", "return_name", "return_type"],
        )
        cursor.execute(query)
        return [
            dict(
...
class GoFunctionsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "go_functions",
            ["file", "line", "name", "signature", "is_exported", "is_async", "doc_comment"],
        )
        cursor.execute(query)
        return [
            dict(
...
class GoGoroutinesTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "go_goroutines", ["file", "line", "containing_func", "spawned_expr", "is_anonymous"]
        )
        cursor.execute(query)
        return [
            dict(
                zip(
                    ["file", "line", "containing_func", "spawned_expr", "is_anonymous"],
                    row,
                    strict=True,
                )
            )
...
class GoImportsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        cursor.execute(query)
        return [
            dict(zip(["file", "line", "path", "alias", "is_dot_import"], row, strict=True))
            for row in cursor.fetchall()
        ]
    ...
    def get_by_file(cursor: sqlite3.Cursor, file: str) -> list[dict[str, Any]]:
        """Get rows by file."""
        query = build_query(
            "go_imports", ["file", "line", "path", "alias", "is_dot_import"], where="file = ?"
        )
        cursor.execute(query, (file,))
        return [
            dict(zip(["file", "line", "path", "alias", "is_dot_import"], row, strict=True))
    ...
    def get_by_path(cursor: sqlite3.Cursor, path: str) -> list[dict[str, Any]]:
        """Get rows by path."""
        query = build_query(
            "go_imports", ["file", "line", "path", "alias", "is_dot_import"], where="path = ?"
        )
...
class GoInterfaceMethodsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "go_interface_methods", ["file", "interface_name", "method_name", "signature"]
        )
        cursor.execute(query)
        return [
            dict(zip(["file", "interface_name", "method_name", "signature"], row, strict=True))
            for row in cursor.fetchall()
        ]
    ...
    def get_by_interface_name(cursor: sqlite3.Cursor, interface_name: str) -> list[dict[str, Any]]:
        ...
        query = build_query(
            ...
            ["file", "interface_name", "method_name", "signature"],
...
class GoInterfacesTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        cursor.execute(query)
        return [
            dict(zip(["file", "line", "name", "is_exported", "doc_comment"], row, strict=True))
            for row in cursor.fetchall()
        ]
    ...
    def get_by_file(cursor: sqlite3.Cursor, file: str) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "go_interfaces",
            ["file", "line", "name", "is_exported", "doc_comment"],
            where="file = ?",
        )
        cursor.execute(query, (file,))
        return [
...
class GoMiddlewareTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "go_middleware",
            ["file", "line", "framework", "router_var", "middleware_func", "is_global"],
        )
        cursor.execute(query)
        return [
            dict(
...
class GoPackagesTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        cursor.execute(query)
        return [
            dict(zip(["file", "line", "name", "import_path"], row, strict=True))
            for row in cursor.fetchall()
        ]
    ...
    def get_by_name(cursor: sqlite3.Cursor, name: str) -> list[dict[str, Any]]:
        """Get rows by name."""
        query = build_query(
            "go_packages", ["file", "line", "name", "import_path"], where="name = ?"
        )
        cursor.execute(query, (name,))
        return [
            dict(zip(["file", "line", "name", "import_path"], row, strict=True))
...
class GoRoutesTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "go_routes", ["file", "line", "framework", "method", "path", "handler_func"]
        )
        cursor.execute(query)
        return [
            dict(
                zip(
                    ["file", "line", "framework", "method", "path", "handler_func"],
                    row,
                    strict=True,
                )
            )
...
class GoStructsTable:
    ...
    def get_by_file(cursor: sqlite3.Cursor, file: str) -> list[dict[str, Any]]:
        """Get rows by file."""
        query = build_query(
            "go_structs", ["file", "line", "name", "is_exported", "doc_comment"], where="file = ?"
        )
        cursor.execute(query, (file,))
        return [
            dict(zip(["file", "line", "name", "is_exported", "doc_comment"], row, strict=True))
    ...
    def get_by_name(cursor: sqlite3.Cursor, name: str) -> list[dict[str, Any]]:
        """Get rows by name."""
        query = build_query(
            "go_structs", ["file", "line", "name", "is_exported", "doc_comment"], where="name = ?"
        )
        cursor.execute(query, (name,))
        return [
            dict(zip(["file", "line", "name", "is_exported", "doc_comment"], row, strict=True))
...
class GoTypeAssertionsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "go_type_assertions",
            ["file", "line", "expr", "asserted_type", "is_type_switch", "containing_func"],
        )
        cursor.execute(query)
        return [
            dict(
...
class GoVariablesTable:
    ...
    @staticmethod
    def get_by_is_package_level(
        cursor: sqlite3.Cursor, is_package_level: bool
    ) -> list[dict[str, Any]]:
...
class RustAsyncFunctionsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            ...
            ["file_path", "line", "function_name", "return_type", "has_await", "await_count"],
        )
        cursor.execute(query)
        return [
            dict(
...
class RustAwaitPointsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "rust_await_points", ["file_path", "line", "containing_function", "awaited_expression"]
        )
        cursor.execute(query)
        return [
            dict(
    ...
    @staticmethod
    def get_by_containing_function(
        cursor: sqlite3.Cursor, containing_function: str
    ) -> list[dict[str, Any]]:
        """Get rows by containing_function."""
        query = build_query(
            "rust_await_points",
            ["file_path", "line", "containing_function", "awaited_expression"],
            where="containing_function = ?",
        )
...
class RustEnumsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "rust_enums",
            ["file_path", "line", "end_line", "name", "visibility", "generics", "derives_json"],
        )
        cursor.execute(query)
        return [
            dict(
...
class RustExternBlocksTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        cursor.execute(query)
        return [
            dict(zip(["file_path", "line", "end_line", "abi"], row, strict=True))
            for row in cursor.fetchall()
        ]
...
class RustExternFunctionsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            ...
            ["file_path", "line", "name", "abi", "return_type", "params_json", "is_variadic"],
        )
        cursor.execute(query)
        return [
            dict(
...
class RustImplBlocksTable:
    ...
    @staticmethod
    def get_by_target_type_raw(
        cursor: sqlite3.Cursor, target_type_raw: str
    ) -> list[dict[str, Any]]:
    ...
    @staticmethod
    def get_by_target_type_resolved(
        cursor: sqlite3.Cursor, target_type_resolved: str
    ) -> list[dict[str, Any]]:
...
class RustLifetimesTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "rust_lifetimes", ["file_path", "parent_line", "lifetime_name", "is_static"]
        )
        cursor.execute(query)
        return [
            dict(zip(["file_path", "parent_line", "lifetime_name", "is_static"], row, strict=True))
            for row in cursor.fetchall()
        ]
...
class RustMacrosTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "rust_macros", ["file_path", "line", "name", "macro_type", "visibility"]
        )
        cursor.execute(query)
        return [
            dict(zip(["file_path", "line", "name", "macro_type", "visibility"], row, strict=True))
            for row in cursor.fetchall()
        ]
...
class RustModulesTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        query = build_query(
            "rust_modules",
            ["file_path", "module_name", "line", "visibility", "is_inline", "parent_module"],
        )
        cursor.execute(query)
        return [
            dict(
...
class RustUnsafeBlocksTable:
    ...
    @staticmethod
    def get_by_has_safety_comment(
        cursor: sqlite3.Cursor, has_safety_comment: bool
    ) -> list[dict[str, Any]]:
...
class RustUnsafeTraitsTable:
    ...
    def get_all(cursor: sqlite3.Cursor) -> list[dict[str, Any]]:
        ...
        cursor.execute(query)
        return [
            dict(zip(["file_path", "line", "trait_name", "impl_type"], row, strict=True))
            for row in cursor.fetchall()
        ]
