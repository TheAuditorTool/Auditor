...
def graph_build_dfg(root, db, repo_db):
    ...
    try:
        ...
        if not repo_db_path.exists():
            console.print(
                f"[error]ERROR: {repo_db} not found. Run 'aud full' first.[/error]",
                stderr=True,
                highlight=False,
            )
            raise click.Abort()
        ...
        console.print(
            f"[error]ERROR: Failed to build DFG: {e}[/error]", stderr=True, highlight=False
        )
        raise click.Abort() from e
...
def graph_analyze(root, db, out, max_depth, workset, no_insights):
    ...
    if not no_insights:
        try:
            ...
            console.print("Note: Insights module not available. Running basic analysis only.")
            insights = None
