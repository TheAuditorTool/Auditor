...
def query(symbol, file, api, component, variable, pattern, category, search,
    ...
    if not any([symbol, file, api, component, variable, pattern, category, search, show_api_coverage]):
        ...
        click.echo("    aud context query --pattern 'auth%' --type-filter function", err=True)
        click.echo("    aud context query --category jwt --format json", err=True)
        click.echo("    aud context query --search payment --include-tables symbols,findings", err=True)
