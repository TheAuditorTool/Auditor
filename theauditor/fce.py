...
def run_fce(
    ...
    if framework_info["name"] != "unknown" and framework_info["cmd"]:
        ...
        tools.append({"name": framework_info["name"], "command": command, "type": "test"})
        ...
        *,
        ...
        if existing:
            ...
            if merge:
                ...
                existing["supporting_count"] = existing.get("supporting_count", 1) + entry.get(
                    "supporting_count", 1
                )
            return False
    ...
    if hotspot_files and consolidated_findings:
        ...
        for hotspot in top_hotspots:
            ...
            if critical_in_hotspot:
                ...
                hotspot_issue_count = sum(f.get("duplicate_count", 1) for f in critical_in_hotspot)
                entry = {
    ...
    if cycles and consolidated_findings:
        for i, cycle in enumerate(cycles[:5]):
            ...
            cycle_findings = [f for f in consolidated_findings if f.get("file") in cycle_files]
            ...
            if len(cycle_findings) >= 5:
                ...
                total_cycle_findings = sum(f.get("duplicate_count", 1) for f in cycle_findings)
                entry = {
        ...
        for finding in consolidated_findings:
            ...
            for _func_key, func_data in complex_functions.items():
                if func_data.get("file") != file_path:
                    continue
                if not (
                    func_data.get("start_line", 0)
                    <= line_num
                    <= func_data.get("end_line", float("inf"))
                ):
                    continue
