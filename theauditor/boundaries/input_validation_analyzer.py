...
def analyze_input_validation_boundaries(db_path: str, max_entries: int = 50) -> list[dict]:
    ...
    try:
        # Find all HTTP routes (primary entry points)
        entry_points = []
        ...
        for entry in entry_points[:max_entries]:
            # Find validation controls reachable from this entry
            ...
            if quality['quality'] == 'missing':
                ...
                # Check if any control is too far
...
def generate_report(analysis_results: list[dict]) -> str:
    ...
    report.append(f"  Acceptable Boundaries: {acceptable} ({acceptable*100//total if total else 0}%)")
