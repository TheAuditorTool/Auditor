...
def extract_protocols(tree: Dict, parser_self) -> List[Dict[str, Any]]:
...
def extract_generics(tree: Dict, parser_self) -> List[Dict[str, Any]]:
...
def extract_typed_dicts(tree: Dict, parser_self) -> List[Dict[str, Any]]:
...
def _get_literal_type_string(annotation_node) -> str:
    ...
    if isinstance(annotation_node.slice, ast.Tuple):
        ...
        for elt in annotation_node.slice.elts:
            ...
            elif isinstance(elt, ast.Num):  # Python 3.7 compat
    ...
    elif isinstance(annotation_node.slice, ast.Str):  # Python 3.7 compat
        parts.append(repr(annotation_node.slice.s))
    elif isinstance(annotation_node.slice, ast.Num):  # Python 3.7 compat
        parts.append(str(annotation_node.slice.n))

    return f"Literal[{', '.join(parts)}]"


def extract_literals(tree: Dict, parser_self) -> List[Dict[str, Any]]:
    ...
    for node in ast.walk(actual_tree):
        ...
        # Variable annotations
            ...
            if node.annotation and _is_literal_annotation(node.annotation):
                literal_type = _get_literal_type_string(node.annotation)
                var_name = get_node_name(node.target)
                literals.append({
                    "line": node.lineno,
                    "usage_context": "variable",
...
def extract_overloads(tree: Dict, parser_self) -> List[Dict[str, Any]]:
