  ...
  for (const call of functionCallArgs) {
    ...
    if (isReactHook || isCustomHook) {
      ...
      react_hooks.push({
        ...
        argument_count:
          call.argument_index !== undefined ? call.argument_index + 1 : 0,
      });
...
function extractVueComponents(vueMeta, filePath, functionCallArgs, returns) {
  ...
  const startLine =
    scriptBlock && scriptBlock.loc ? scriptBlock.loc.start.line : 1;
  const endLine =
    scriptBlock && scriptBlock.loc ? scriptBlock.loc.end.line : startLine;
  ...
  grouped.forEach((args) => {
    ...
    if (
      ...
      hooks.push({
        ...
        hook_type: hookType,
        dependencies:
          dependencyArg && dependencyArg.argument_expr
            ? truncateVueString(dependencyArg.argument_expr)
            : null,
        return_value:
          handlerArg && handlerArg.argument_expr
            ? truncateVueString(handlerArg.argument_expr)
            : null,
        is_async: Boolean(
          handlerArg &&
          handlerArg.argument_expr &&
          handlerArg.argument_expr.trim().startsWith("async"),
        ),
      });
    }
  });
...
function extractVueProvideInject(functionCallArgs, componentName) {
  ...
  grouped.forEach((args) => {
    ...
    records.push({
      ...
      is_reactive: Boolean(
        valueExpr && /ref\s*\(|reactive\s*\(/.test(valueExpr),
      ),
    });
  });
...
function extractNestJSResolvers(
  functions,
  classes,
  func_decorators,
  func_decorator_args,
    ...
    for (const method of classMethods) {
      ...
      for (const decorator of methodDecorators) {
        ...
        if (
          ...
          const decArgs = func_decorator_args.filter(
            (a) =>
...
function extractTypeGraphQLResolvers(
  functions,
  classes,
  func_decorators,
  func_decorator_args,
  ...
  for (const cls of classes) {
    ...
    for (const method of classMethods) {
      ...
      for (const decorator of methodDecorators) {
        ...
        if (
          ["Query", "Mutation", "Subscription", "FieldResolver"].includes(
            decoratorName,
          )
        ) {
          let fieldName = method.name.split(".").pop();
          ...
          const methodParamDecorators = func_param_decorators.filter(
            (pd) =>
              pd.function_name === method.name &&
              pd.function_line === method.line &&
              pd.decorator_name === "Arg",
          );
          const argParamIndices = new Set(
            methodParamDecorators.map((pd) => pd.param_index),
          );
