function serializeNodeForCFG(node, sourceFile, ts, depth = 0, maxDepth = 100) {
  ...
  ts.forEachChild(node, (child) => {
    const serializedChild = serializeNodeForCFG(
      child,
      sourceFile,
      ts,
      depth + 1,
  ...
  if (node.initializer) {
    serialized.initializer = serializeNodeForCFG(
      node.initializer,
      sourceFile,
      ts,
      depth + 1,
  ...
  if (node.condition) {
    serialized.condition = serializeNodeForCFG(
      node.condition,
      sourceFile,
      ts,
      depth + 1,
  ...
  if (node.expression) {
    serialized.expression = serializeNodeForCFG(
      node.expression,
      sourceFile,
      ts,
      depth + 1,
              ...
              if (decorator.expression) {
                if (decorator.expression.kind === ts.SyntaxKind.Identifier) {
                  decoratorName =
                    ...
                    decorator.expression.escapedText ||
                    "";
                } else if (
                  ...
                  if (
                    ...
                    decoratorName =
                      ...
                      callExpr.expression.escapedText ||
                      "";
                  }
  ...
  function traverse(node) {
    ...
    if (kind === "ClassDeclaration") {
      ...
      if (node.heritageClauses) {
        for (const clause of node.heritageClauses) {
          if (
            ...
            classEntry.extends_type = extendsType.expression
              ? extendsType.expression.text ||
                extendsType.expression.escapedText
              : null;
          }
        }
      }
      ...
      if (node.typeParameters && node.typeParameters.length > 0) {
        ...
        classEntry.type_params = node.typeParameters
          .map((tp) => {
            const paramName = tp.name
              ? tp.name.text || tp.name.escapedText
              : "T";
            if (tp.constraint) {
...
function extractClassProperties(sourceFile, ts) {
  ...
  function traverse(node) {
    ...
    if (kind === "PropertyDeclaration" && currentClass) {
      ...
      if (node.initializer) {
        property.initializer = node.initializer
          .getText(sourceFile)
          .substring(0, 500);
      }
...
function buildScopeMap(sourceFile, ts) {
  ...
  function collectFunctions(node, depth = 0, parent = null) {
    ...
    let funcName = null;
    let actualFunctionNode = node;

    if (kind === "FunctionDeclaration") {
      funcName = node.name
        ? node.name.text || node.name.escapedText || "anonymous"
        : "anonymous";
    } else if (kind === "MethodDeclaration") {
