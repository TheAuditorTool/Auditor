/**
 * Comprehensive test suite for Node-side fidelity manifest generation.
 *
 * Tests cover:
 * 1. createManifest() - token generation from rows
 * 2. attachManifest() - integration with extraction results
 * 3. Byte calculation parity with Python
 * 4. Edge cases and boundary conditions
 *
 * Run with: npx tsx src/fidelity.test.ts
 */

import { createManifest, attachManifest, FidelityManifest } from './fidelity.js';
...
test('createManifest tx_id is valid UUID format', () => {
  ...
  assertTrue(uuidRegex.test(manifest.tx_id), `Invalid UUID format: ${manifest.tx_id}`);
});
...
// =============================================================================
// SECTION 2: createManifest() - Byte Calculation (Python Parity)
// =============================================================================

console.log('\n=== SECTION 2: createManifest() Byte Calculation ===\n');

test('createManifest bytes from simple string values', () => {
  // Python: sum(len(str(v)) for row in rows for v in row.values())
  // "hello" = 5 chars, "100" = 3 chars -> 8 bytes
...
test('createManifest bytes handles nested objects', () => {
  // String(nested) will be "[object Object]" = 15 chars
...
test('createManifest bytes handles boolean', () => {
  // String(true) = "true" = 4, String(false) = "false" = 5
...
// =============================================================================
// SECTION 3: createManifest() - Empty and Edge Cases
// =============================================================================
...
test('createManifest non-array returns zero token', () => {
  // TypeScript enforces array, but runtime could receive anything
...
test('attachManifest creates manifest for each table', () => {
  ...
  const withManifest = attachManifest(results);
  const manifest = withManifest['/test/file.ts'].extracted_data._extraction_manifest;
...
test('attachManifest skips non-array values', () => {
  const results = {
    '/test/file.ts': {
      ...
      extracted_data: {
        symbols: [{ name: 'foo' }],
        metadata: { version: '1.0' },  // Not an array
        count: 42,  // Not an array
      },
    },
  };
...
test('attachManifest handles multiple files', () => {
  ...
  const manifest1 = withManifest['/file1.ts'].extracted_data._extraction_manifest;
  const manifest2 = withManifest['/file2.ts'].extracted_data._extraction_manifest;
...
test('manifest token has correct structure for reconciliation', () => {
  ...
  const withManifest = attachManifest(results);
  const token = withManifest['/test/file.ts'].extracted_data._extraction_manifest.symbols;

  // Validate structure matches Python FidelityToken.create_receipt expectations
...
test('manifest columns reflect first row schema', () => {
  ...
  const withManifest = attachManifest(results);
  const token = withManifest['/test/file.ts'].extracted_data._extraction_manifest.data;

  // Columns should match first row's keys, sorted
...
test('byte calculation matches Python FidelityToken.create_manifest', () => {
  // This test ensures Node byte calculation matches Python exactly
  // Python: sum(len(str(v)) for row in rows for v in row.values())
  const rows = [
  ...
  // Python calculation:
  // Row 1: len("hello")=5 + len("function")=8 + len("10")=2 = 15
  // Row 2: len("world")=5 + len("class")=5 + len("20")=2 = 12
  // Total: 27
...
test('numeric values stringified correctly', () => {
  const rows = [
    { int_val: 12345, float_val: 3.14159 },
  ];
  const manifest = createManifest(rows);

  // "12345" = 5, "3.14159" = 7 -> 12
