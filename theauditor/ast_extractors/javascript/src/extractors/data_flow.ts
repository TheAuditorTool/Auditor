 ...
 * Data Flow Extractor
 *
 * Extracts function calls, assignments, returns, object literals, and variable usage.
 *
 * CHANGES FROM LEGACY data_flow.js:
 * - REWRITTEN: extractCalls uses TypeChecker for semantic symbol resolution
 *   - Returns name (resolved), original_text (raw), defined_in (file path)
 *   - Fixes the "anonymous caller" bug where db.User.findAll() couldn't be traced
...
// =============================================================================
// EXTRACT CALLS - REWRITTEN WITH TYPECHECKER (spec2.md Section 3)
// =============================================================================

/**
 * Extracts function calls with SEMANTIC analysis using TypeChecker.
 *
 * NEW SEMANTIC FIELDS (from spec2.md Section 8):
 * - name: Resolved symbol name (e.g., "User.findAll" not "db.users.findAll")
 * - original_text: Raw text from AST (for debugging)
 * - defined_in: File path where function is defined
 *
 * FIXES: The "anonymous caller" bug where text-based buildName() couldn't
 * resolve db.User.findAll() to the actual User model from ./models
 */
export function extractCalls(
  ...
  // Helper to build name from expression (fallback when no checker)
  ...
  function traverse(node: ts.Node): void {
    ...
    if (kind === 'CallExpression') {
      ...
      // SEMANTIC EXTRACTION using TypeChecker (spec2.md Section 3)
      if (checker) {
        ...
        if (symbol) {
          ...
          // Fallback to text if semantic analysis fails
          // CRITICAL FIX: Ensure we never return empty string
          const builtName = buildName(callExpr.expression);
          resolvedName = builtName || originalText || 'unknown';
        }
      } else {
        // No checker - use text-based extraction
        // CRITICAL FIX: Ensure we never return empty string
        const builtName = buildName(callExpr.expression);
        resolvedName = builtName || originalText || 'unknown';
      }

      // Extract arguments as text
      const args = callExpr.arguments.map((arg) =>
        arg.getText(sourceFile).substring(0, 500)
      );

      // Get caller function from scope map
      ...
      // Also extract property accesses as potential call-like symbols
      const pae = node as ts.PropertyAccessExpression;
      let fullName = '';

      // Text-based extraction for property accesses
      ...
      if (name && initializer) {
        if (name.kind === ts.SyntaxKind.Identifier) {
          ...
          pattern.elements.forEach((elem) => {
            ...
            if (bindingElem.name && bindingElem.name.kind === ts.SyntaxKind.Identifier) {
        ...
        if (exprKind === 'PropertyAccessExpression') {
          ...
          if (pae.expression) {
            ...
            if (innerExprKind === 'CallExpression') {
              const innerNodeId = pae.expression.pos + '-' + pae.expression.kind;
          ...
          const innerNodeId = callExpr.expression.pos + '-' + callExpr.expression.kind;
          if (!visited.has(innerNodeId)) {
    ...
    if (node.kind === ts.SyntaxKind.CallExpression) {
      ...
      if (checker && callExpr.expression) {
        try {
          ...
          // Cross-file resolution can fail for external modules
        }
      }
  ...
  properties.forEach((prop) => {
    ...
    if (kind === 'PropertyAssignment') {
      ...
      const propName = shorthand.name
        ? shorthand.name.text || shorthand.name.escapedText?.toString() || '<unknown>'
        : '<unknown>';
