...
**Framework:** This agent follows the Phase → Task → Job hierarchy with problem decomposition thinking (see planning.md for structure details). All phases include "Success Criteria" fields, all tasks end with audit jobs, and all phases end with phase audit tasks.
...
**Success Criteria:** Prevents wasting time analyzing deprecated/orphaned files that should be deleted, not refactored. Separates cleanup tasks from refactor tasks.
...
**Success Criteria:** Prevents inventing new patterns when established conventions exist. Ensures consistency with existing codebase structure (schemas/ split, commands/ split, etc.).
...
**Success Criteria:** Provides factual basis for split decisions using database queries instead of file reading. Identifies natural clustering (prefix patterns, domain groupings) from actual code structure.
...
**Success Criteria:** Files >2150 lines (~80KB, ~25k tokens) cannot be read in one shot due to context limits. Chunked reading is REQUIRED for detailed inspection of implementation patterns, imports, and code structure.
...
**Success Criteria:** Identifies ambiguous split states (10-90% complete) where intent is unclear. Prevents making wrong assumption about whether to FINISH split or REVERT split. Forces explicit user direction.
...
**Success Criteria:** Prevents re-analyzing files that were recently checked. Respects previous decisions (HIGH risk = review migrations first). Provides continuity across refactor sessions.
...
**Success Criteria:** Removes agent bias and assumptions. Provides user with complete factual picture to make informed decision. Follows Zero Recommendation Policy - facts only, no suggestions.
...
7. **Problem Decomposition:** Each phase solves specific sub-problem with "Success Criteria" justification
