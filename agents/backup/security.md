...
**Success Criteria:** Prevents recommending joi when project uses zod, or marshmallow when project uses pydantic. Ensures security mitigations follow detected framework patterns instead of inventing new validation approaches.
...
**Success Criteria:** Provides current security posture before running new analysis. Identifies which files have most findings, what vulnerability types exist, and establishes baseline for improvement tracking.
...
**Success Criteria:** Identifies actual code locations vulnerable to attacks using database queries instead of file reading. Provides factual basis for security recommendations with line numbers and file paths.
...
**Success Criteria:** Finds routes accepting user input without validation. Uses detected validation library (zod/joi/marshmallow) to identify coverage gaps. Ensures recommendations match existing validation patterns.
...
**Success Criteria:** Provides user with complete security picture using only database facts. Ensures all recommendations match detected frameworks (zod if zod detected, Sequelize if Sequelize detected). No invented patterns.
