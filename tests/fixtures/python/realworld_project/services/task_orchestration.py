...
def execute_task_group(report_ids):
    ...
    task_group = group(generate_report.s(report_id) for report_id in report_ids)
    task_group.apply_async()


def execute_chord_pattern(user_ids):
    """Send emails to multiple users, then send summary SMS."""
    email_tasks = group(send_email.s(user_id, "Newsletter", "Content") for user_id in user_ids)
    callback = send_sms.s("+1234567890", "All emails sent")

    task_chord = chord(email_tasks)(callback)
...
send_email.delay(999, "System", "Startup notification")


def synchronous_task_execution(user_id):
    """Execute task synchronously for testing."""
    result = send_email.apply(args=(user_id, "Test", "Body"))
    return result
...
def batch_email_sender(user_list):
    ...
    for user in user_list:
        send_email.delay(user["id"], user["subject"], user["body"])
...
def mixed_invocation_patterns(user_id, urgent=False):
    ...
    if urgent:
        send_sms.apply_async(args=("+1234567890", "URGENT"), queue="high_priority", countdown=0)
