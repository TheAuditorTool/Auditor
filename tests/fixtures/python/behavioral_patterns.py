...
def tail_factorial(n, acc=1):
    ...
    if n <= 0:  # Base case
        return acc
    return tail_factorial(n - 1, n * acc)  # Tail recursion (last operation is recursive call)
...
def fibonacci(n):
    ...
    if n == 1:
        return 1
    return fibonacci(n - 1) + fibonacci(n - 2)  # Direct recursion (twice)
...
async def async_countdown(n):
    ...
    if n <= 0:
        return
    await async_countdown(n - 1)  # Async recursion


# ============================================================================
# PATTERN 2: Generator Yields
# ============================================================================

def simple_generator():
    """Test simple yield."""
    yield 1  # Simple yield
    yield 2
    yield 3
...
def conditional_generator(n):
    ...
    for i in range(n):
        if i % 2 == 0:
            yield i  # Conditional yield inside loop


def generator_with_loop():
    """Test yield in loop."""
    for x in [1, 2, 3, 4, 5]:
        yield x * 2  # Yield in loop


def yield_from_example():
    """Test yield from delegation."""
    yield from range(10)  # Yield from
    yield from simple_generator()  # Yield from another generator
...
def filtered_generator(data):
    ...
    for item in data:
        if item > 0:
            yield item  # Conditional yield in loop
...
def nested_loop_generator():
    ...
    for i in range(3):
        for j in range(3):
            yield (i, j)  # Yield in nested loop
...
class Rectangle:
    ...
    def width(self):
        ...
        return self._width  # Simple return (no computation)

    @width.setter
    def width(self, value):
        """Property setter with validation."""
        if value < 0:
            raise ValueError("Width cannot be negative")  # Validation
        self._width = value

    @property
    def area(self):
        """Computed property (has computation)."""
        return self._width * self._height  # Computed property (BinOp)

    @property
    def perimeter(self):
        """Another computed property."""
        return 2 * (self._width + self._height)  # Computation

    @area.setter
    def area(self, value):
        """Property setter without validation (just assignment)."""
        self._area_cache = value  # No validation
...
class User:
    ...
    def age(self, value):
        ...
        if value < 0 or value > 150:
            raise ValueError("Invalid age")  # Validation
        self._age = value
...
class DynamicDict:
    ...
    def __getattr__(self, name):
        ...
        return self._data.get(name)  # Delegation to _data
    ...
    def __setattr__(self, name, value):
        ...
        if name == "_data":
            ...
            self._data[name] = value  # Delegation to _data
    ...
    def __setattr__(self, name, value):
        ...
        if name == "_validated":
            ...
            if not isinstance(value, (int, str)):
                raise TypeError("Only int and str allowed")  # Validation
...
class ProxyClass:
    ...
    def __getattribute__(self, name):
        ...
        return getattr(wrapped, name)  # Delegation to wrapped object
...
class ResourceManager:
    ...
    def __delattr__(self, name):
        ...
        if name in self._resources:
            self._resources[name].cleanup()  # Cleanup before deletion
...
class TreeNode:
    ...
    def height(self):
        ...
        if self._left is None and self._right is None:
            return 0
        left_height = self._left.height if self._left else 0  # Recursive property access
        right_height = self._right.height if self._right else 0
        return 1 + max(left_height, right_height)  # Computation

    def inorder_traversal(self):
        """Generator with recursion."""
        if self._left:
            yield from self._left.inorder_traversal()  # Recursive yield from
        yield self._value  # Yield
        if self._right:
            yield from self._right.inorder_traversal()  # Recursive yield from
