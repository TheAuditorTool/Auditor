        ...
        validate: {
          isValidDimensions(value) {
            if (value && (!value.length || !value.width || !value.height)) {
              throw new Error(
                "Dimensions must contain length, width, and height",
              );
            }
          },
        },
      ...
      hooks: {
        ...
        afterUpdate: async (product, options) => {
          if (product.changed("price")) {
            ...
            console.log(
              `Product ${product.sku} price changed by ${changePercent}%: $${oldPrice} → $${newPrice}`,
            );
          }
        },
      },
  ...
  Product.getInventoryValue = async function () {
    ...
    const result = await this.findAll({
      attributes: [
        ...
        [
          fn("SUM", fn("*", col("stockQuantity"), col("price"))),
          "totalRetailValue",
        ],
