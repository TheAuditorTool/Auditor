...
# JSON blob columns that should NOT exist (replaced with junction tables)
FORBIDDEN_NODE_JSON_COLUMNS = {
    # Vue - removed from vue_components
    'props_definition',
    'emits_definition',
    'setup_return',
    # Angular - removed from angular_components
    'style_paths',
    # Angular - removed from angular_modules
    'declarations',
    'imports',
    'providers',
    'exports',
...
# Junction table column specifications
...
# Database dispatcher methods added in node-schema-normalization
...
class TestDatabaseDispatcherMethods:
    ...
    def test_junction_dispatcher_methods_exist(self):
        ...
        assert not missing, (
            f"NodeDatabaseMixin missing dispatcher methods: {missing}"
        )
    ...
    def test_dispatcher_methods_are_callable(self):
        ...
        for method_name in JUNCTION_DISPATCHER_METHODS:
            ...
            assert callable(method), (
                f"NodeDatabaseMixin.{method_name} is not callable"
            )
...
class TestGenericBatchesConfiguration:
    ...
    def test_generic_batches_accepts_junction_keys(self):
        ...
        # Verify generic_batches exists and is a defaultdict
        assert hasattr(manager, 'generic_batches'), (
            "DatabaseManager missing generic_batches attribute"
        )

        # Test that we can append to junction table keys without KeyError
        # (defaultdict(list) behavior)
        for table_name in NODE_JUNCTION_TABLES:
            try:
    ...
    def test_parent_table_columns_reduced(self):
        ...
        # vue_components: was 11 columns (with JSON), now 8
        vue_cols = len(VUE_COMPONENTS.columns)
        assert vue_cols == 8, (
            f"vue_components should have 8 columns (JSON removed), got {vue_cols}"
        )

        # angular_components: was 7 columns (with style_paths), now 6
        ...
        # angular_modules: was 7 columns (with 4 JSON arrays), now 3
