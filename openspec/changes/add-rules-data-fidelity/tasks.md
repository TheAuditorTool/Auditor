...
- [ ] 0.1 Verify `theauditor/indexer/schema.py:20` exports `TABLES: dict[str, TableSchema]`
- [ ] 0.2 Verify `TableSchema` at `schemas/utils.py:74` has `column_names()` method returning `list[str]`
- [ ] 0.3 Verify `TableSchema.foreign_keys` is `list[ForeignKey]` with `local_columns`, `foreign_table`, `foreign_columns`
- [ ] 0.4 Verify `theauditor/rules/base.py:83` has `StandardFinding` dataclass
- [ ] 0.5 Verify `theauditor/rules/base.py:29` has `StandardRuleContext` dataclass
- [ ] 0.6 Verify `theauditor/rules/orchestrator.py:483` has `_execute_rule()` method
- [ ] 0.7 Verify orchestrator line 490 is where `rule.function(std_context)` is called
...
- [ ] 1.1 Create `Q` class with constructor accepting table name
  ```python
...
- [ ] 1.2 Implement `.select(*columns)` method
  - Store column list (validation deferred to build)
  - Return `self` for chaining

- [ ] 1.3 Implement `.where(condition, *params)` method
  - Store condition string and params
  - Support multiple `.where()` calls (AND together)
  - Return `self` for chaining

- [ ] 1.4 Implement `.join(table, on=None, join_type="INNER")` method
  - Store join config
  - `on` formats:
...
- [ ] 1.5 Implement `.with_cte(name, subquery: Q)` method
  - Store CTE name and Q object
  - Support multiple CTEs (stored in order)
  - Return `self` for chaining

- [ ] 1.6 Implement `.order_by(clause)` method
  - Store order clause as string
  - Return `self` for chaining

- [ ] 1.7 Implement `.limit(n)` method
  - Store limit value as int
  - Return `self` for chaining

- [ ] 1.8 Implement `.group_by(*columns)` method
  - Store group by columns
  - Return `self` for chaining

- [ ] 1.9 Implement `.build()` method - Core Logic
  ```python
...
- [ ] 1.10 Implement `_find_fk(base_table, join_table)` helper for FK auto-detection
  ```python
...
- [ ] 1.11 Implement `Q.raw(sql, params)` class method
  ```python
  @classmethod
...
- [ ] 2.1 Create `RuleResult` dataclass
  ```python
...
- [ ] 2.2 Create `RuleManifest` helper class
  ```python
  class RuleManifest:
...
- [ ] 2.3 Create `RuleDB` helper class
  ```python
  class RuleDB:
...
- [ ] 2.4 Create `FidelityError` exception class
  ```python
...
- [ ] 2.5 Create fidelity verification function
  ```python
  import os
...
- [ ] 3.2 Add `_fidelity_failures` list to `__init__`
  ```python
  self._fidelity_failures: list[tuple[str, list[str]]] = []
  ```

- [ ] 3.3 Update `_execute_rule()` at line 486-499 to handle `RuleResult`
  ```python
  # BEFORE (line 490):
...
- [ ] 3.4 Add `_compute_expected()` method
  ```python
...
- [ ] 3.5 Add `get_aggregated_manifests()` method
  ```python
...
- [ ] 4.1 Create `__all__` list at top of file (does not exist currently)
  ```python
  __all__ = [
      "Severity",
      "Confidence",
...
- [ ] 4.2 Add import and re-export of RuleResult
  ```python
  from theauditor.rules.fidelity import RuleResult
  ```

- [ ] 4.3 Update `RuleFunction` type hint at line 132
  ```python
  # BEFORE:
...
- [ ] 6.1 Add module docstring to query.py with usage examples
- [ ] 6.2 Add module docstring to fidelity.py with usage examples
- [ ] 6.3 Add inline comments showing before/after migration patterns

## 7. Validation

- [ ] 7.1 Run `openspec validate add-rules-data-fidelity --strict`
- [ ] 7.2 Run all tests: `pytest tests/rules/test_query.py tests/rules/test_fidelity.py -v`
- [ ] 7.3 Manually test Q class against real `repo_index.db`:
  ```python
  ...
  print(params)
  ```
- [ ] 7.4 Convert one existing rule as proof of concept (suggest: `ghost_dependencies.py` - simple tier 1)
